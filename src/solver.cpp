...
void FRSolver::check_blanking(void)
{

}

void FRSolver::process_blanks(void)
{
#ifdef _MPI
  /* --- Set blank/unblank faces for all elements to be blanked --- */
  std::unordered_set<int> blankIFaces, blankMFaces, blankOFaces, ubOFaces;
  for (auto &ic:geo.blankCells)
  {
    if (geo.eleMap[ic] < 0) continue; // Ignore already-blanked cells
    ...
    for (int j = 0; j < geo.nFacesPerEle; j++)
    {
      ...
      switch (ftype)
      {
        ...
        case HOLE_FACE:
          break;
        default:
          ThrowException("Face of blankCell has unknown currFaceType.");
          break;
      }
    }
  }
  ...
  std::vector<int> blankMpi;
  for (auto &ff:blankMFaces) blankMpi.push_back(ff);

  int nblankMpi = blankMpi.size();
  std::vector<int> nblankMpi_rank(input->nRanks);
  MPI_Allgather(&nblankMpi,1,MPI_INT,nblankMpi_rank.data(),1,MPI_INT,myComm);

  int sum = 0;
  std::vector<int> recvCnts(input->nRanks);
  std::vector<int> recvDisp(input->nRanks);
  for (int i = 0; i < input->nRanks; i++)
  {
  ...
  std::vector<int> blankMpi_rank(sum);
  MPI_Allgatherv(blankMpi.data(),blankMpi.size(),MPI_INT,blankMpi_rank.data(),recvCnts.data(),recvDisp.data(),MPI_INT,myComm);

  for (int F = 0; F < geo.nMpiFaces; F++)
  {
    int ff = geo.mpiFaces[F];
    int p = geo.procR[F];
    int f2 = geo.faceID_R[F];
  ...
  std::unordered_set<int> ubIFaces, ubMFaces;

  remove_elements(geo.blankCells);
  remove_faces(blankIFaces,blankMFaces,blankOFaces);
  insert_faces(ubIFaces,ubMFaces,ubOFaces);

  // do any setup ops on faces
#endif
}

void FRSolver::process_unblanks(void)
{
  /* --- Set Unblank/Blank Faces for All Unblank Elements --- */

  std::unordered_set<int> ubIntFaces, ubMpiFaces, ubOFaces;
  std::unordered_set<int> blankIFaces, blankMFaces, blankOFaces;
  ...
  std::unordered_set<int> allMpiFaces = ubMpiFaces;
  for (auto &ff:geo.overFaces) if (geo.faceType[ff] == MPI_FACE) allMpiFaces.insert(ff);

  std::vector<int> ubMpi;
  for (auto &ff:allMpiFaces) ubMpi.push_back(ff);

  int nUbMPI = ubMpi.size();
  std::vector<int> nubMpi_rank(input->nRanks);
  MPI_Allgather(&nUbMPI,1,MPI_INT,nubMpi_rank.data(),1,MPI_INT,myComm);

  int sum = 0;
  vector<int> recvCnts(input->nRanks);
  vector<int> recvDisp(input->nRanks);
  for (int i=0; i<input->nRanks; i++) {
  ...
  std::vector<int> ubMpi_rank(sum);
  MPI_Allgatherv(ubMpi.data(),ubMpi.size(),MPI_INT,ubMpi_rank.data(),recvCnts.data(),recvDisp.data(),MPI_INT,myComm);

  for (int F = 0; F < geo.mpiFaces.size(); F++) {
    int ff = geo.mpiFaces[F];
    if (ubMpiFaces.count(ff) || geo.overFaces.count(ff)) {
      int p = geo.procR[F];
      int f2 = geo.faceID_R[F];
      ...
      if (ubMpiFaces.count(ff) && !isUnblanked) {
        ...
        ubMpiFaces.erase(ff);
        ubOFaces.insert(ff);
      } else if (geo.currFaceType[ff] == OVER_FACE && isUnblanked) {
  ...
  insert_elements(geo.unblankCells);
  remove_faces(blankIFaces,blankMFaces,blankOFaces);
  insert_faces(ubIntFaces,ubMpiFaces,ubOFaces);

  // Do any setup required for new elements / faces
}

void FRSolver::remove_elements(std::unordered_set<int> blankEles)
{
  /* --- Remove Newly-Blanked Elements --- */

  for (auto &ic:blankEles) {
    if (ic < 0) continue;
    int ind = geo.eleMap[ic];
    if (ind < 0) continue; //FatalError("Should not have marked a hole cell for blanking!");
    /// TODO eles.erase(eles.begin()+ind,eles.begin()+ind+1);
    geo.eleMap[ic] = -1;

    // Update the map
    for (int k = ic+1; k < geo.nEles; k++)
      if (geo.eleMap[k] >= 0)
        geo.eleMap[k]--;
  }
}

void FRSolver::insert_elements(std::unordered_set<int> ubEles)
{
  ...
  for (auto &ic:ubEles) {
    ...
    int ind = geo.eleMap[ic];
    if (ind >= 0) ThrowException("Should not have marked a non-hole cell for un-blanking! Is eleMap wrong?");
    int j = 1;
    while (ind < 0 && j<=ic) {
      ind = geo.eleMap[ic-j];
      j++;
...
void FRSolver::remove_faces(std::unordered_set<int> blankIFaces,
                            ...
                            std::unordered_set<int> blankOFaces)
{
  ...
  for (auto &ff:blankIFaces) {
    ...
    int ind = geo.faceMap[ff];
    int fType = geo.currFaceType[ff];
    if (ind < 0) ThrowException("invalid blankIFace!");

    if (fType == INTERNAL || fType == BOUNDARY) {
//      faces.erase(faces.begin()+ind,faces.begin()+ind+1);

      geo.faceMap[ff] = -1;
      geo.currFaceType[ff] = HOLE_FACE;
//      for (int i = 0; i < faces.size(); i++) geo.faceMap[faces[i]->ID] = i;
  ...
  for (auto &ff:blankMFaces) {
    if (ff < 0) continue;
    int ind = geo.faceMap[ff];
    if (ind < 0) ThrowException("Invalid blankMFace!");

//    mFaces.erase(mFaces.begin()+ind,mFaces.begin()+ind+1);

    geo.faceMap[ff] = -1;
    geo.currFaceType[ff] = HOLE_FACE;
//    for (int i=0; i<mFaces.size(); i++) geo.faceMap[mFaces[i]->ID] = i;
  }
...
//    oFaces.erase(oFaces.begin()+ind,oFaces.begin()+ind+1);
    ...
    geo.faceMap[ff] = -1;
    geo.currFaceType[ff] = HOLE_FACE;
//    for (int i=0; i<oFaces.size(); i++) geo.faceMap[oFaces[i]->ID] = i;

    geo.overFaces.erase(ff);
  }
}

void FRSolver::insert_faces(std::unordered_set<int> ubIFaces, std::unordered_set<int> ubMFaces, std::unordered_set<int> ubOFaces)
{
...
//        ic2 = geo.eleMap[ic2];
        if (ic1<0 || ic2<0)
          ThrowException("Unblanking an internal face, but an ele remains blanked!");
...
//      faces.insert(faces.begin()+ind,1,iface);
      geo.faceMap[ff] = ind;
      geo.currFaceType[ff] = INTERNAL;
      geo.nIntFaces++;
//      for (int i=ind+1; i<faces.size(); i++)
//        geo.faceMap[faces[i]->ID] = i;
    }
    else if (geo.faceType[ff] == BOUNDARY)
    {
      int ind = findFirst(geo.bndFaces, ff);

      if (geo.bcType[ind] == OVERSET) {
...
//        shared_ptr<face> bface = make_shared<boundFace>();
        ...
        {
//          struct faceInfo info;
//          info.bcType = geo.bcType[ind];
//          info.isBnd = 1;
          ic = geo.eleMap[ic];
          if (ic<0) ThrowException("Unblanking a boundary face, but ele remains blanked!");
...
//          bface->setupFace();
        ...
        {
          ind = geo.faceMap[f2];
//          if ( ind>0 && (geo.currFaceType[f2] == INTERNAL || geo.currFaceType[f2] == BOUNDARY) && faces[ind]->ID<ff )
//          {
//            ind++;
//            break;
//          }
...
//          geo.faceMap[faces[i]->ID] = i;
  ...
  for (auto &ff:ubMFaces) {
    int ind = findFirst(geo.mpiFaces, ff);
...
//        relRot = compareOrientationMPI(ic,fid1,geo.gIC_R[ind],geo.mpiLocF_R[ind],geo.mpiPeriodic[ind]);
      }
//      struct faceInfo info;
//      info.IDR = geo.faceID_R[ind];
//      info.relRot = relRot;
//      info.procL = input->rank;
//      info.procR = geo.procR[ind];
//      info.isMPI = 1;
//      info.gridComm = myComm;  // Note that this is equivalent to MPI_COMM_WORLD if non-overset (ngrids = 1)
      ic = geo.eleMap[ic];
      if (ic<0) ThrowException("Unblanking an MPI face, but ele remains blanked!");
//      shared_ptr<ele> nullEle;
...
//    mFaces.insert(mFaces.begin()+ind,1,mface);
    geo.faceMap[ff] = ind;
    geo.currFaceType[ff] = MPI_FACE;
//    for (int i=ind+1; i<mFaces.size(); i++)
//      geo.faceMap[mFaces[i]->ID] = i;
  }
#endif
...
//    struct faceInfo info;
    ic = geo.eleMap[ic];
    if (ic<0) ThrowException("Unblanking an Overset face, but ele remains blanked!");
//    shared_ptr<ele> nullEle;
...
//    oFaces.insert(oFaces.begin()+ind,1,oface);
    geo.faceMap[ff] = ind;
    geo.currFaceType[ff] = OVER_FACE;
//    for (int i=ind+1; i<oFaces.size(); i++)
//      geo.faceMap[oFaces[i]->ID] = i;

    // Add this face to list of overFaces (while keeping list sorted)
    geo.overFaces.insert(ff);
  }
  geo.overFaces.erase(-1);
