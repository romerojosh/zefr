...
void Filter::setup_vandermonde_matrices()
{
  ...
  std::vector<double> loc(eles->nDims);
...
double Filter::calc_expfilter_coeffs(int in_mode, int type)
{
...
void Filter::setup_expfilter_matrix(void)
{
...
#ifdef _USE_KA // Kartikey's Version
  // Calculate KS
  auto &A = oppS(0,0);
  auto &B = U_spts(0, 0, 0);
  auto &C = KS(0, 0, 0);
...
#ifdef _USE_KA
  compute_max_sensor_wrapper_ka(KS_d, sensor_d, order, max_sensor_d, eles->nSpts, eles->nEles, eles->nVars);
#else
...
void Filter::apply_expfilter_type2()
{
#ifdef _CPU
  auto &A = filt2(0,0);
  auto &B = eles->U_spts(0, 0, 0);
...
#pragma omp parallel for collapse(2)    
  ...
  // Copy back filtered values
  copy_filtered_solution_wrapper_ka(U_spts_d, eles->U_spts_d, sensor_d, threshJ, eles->nSpts, eles->nEles, eles->nVars);
