...
Filter::Filter(InputStruct *input, GeoStruct *geo, Elements *eles, Faces *faces)
{
  this->input = input;
  this->geo = geo;
  this->eles = eles;
...
void Filter::setup()
{
  ...
  setup_threshold();
  setup_reshapeOp(); 
	sensor.assign({eles->nEles, eles->nVars});
  u.assign({eles->nSpts});
...
void Filter::setup_threshold()
{
  ...
  for (unsigned int spt = 0; spt < eles->nSpts1D; spt++)
    u_canon(spt, 0) = -std::signbit(eles->loc_spts_1D(spt));
    
  // Ramp in parent domain
  for (unsigned int spt = 0; spt < eles->nSpts1D; spt++)
    u_canon(spt, 1) = -std::signbit(eles->loc_spts_1D(spt)) *eles->loc_spts_1D(spt);
  ...
  for (unsigned int spt = 0; spt < eles->nSpts1D; spt++)
  {
    KS_step(spt, 0) = order * dabs(KS(spt, 0)) * dabs(KS(spt, 0));
    KS_ramp(spt, 1) = order * dabs(KS(spt, 1)) * dabs(KS(spt, 1));
  }
...
void Filter::setup_reshapeOp()
{
  ...
  {
    std::cout << "Sensor on hexes not yet implemented." << std::endl;
  }  
}


void Filter::apply_sensor_ele(unsigned int ele, unsigned int var)
{
  // Initialize sensor output
  sensor.fill(0.0);
  ...
  uMin = u.min_val();
  for (unsigned int spt = 0; spt < eles->nSpts; spt++)
    u(spt) = (u(spt) - minU) / (uMax - uMin + 1e-10);
  ...
  for (unsigned int j = 0; j < 2 * eles->nSpts1D; j++)
    for (unsigned int i = 0; i < eles->nSpts1D; i++)
      KS_lines(i, j) = order * dabs(KS_lines(i, j)) * dabs(KS_lines(i, j));
    
  // Store sensor values
  sensor(ele, var) = KS_lines.max_val();
}
