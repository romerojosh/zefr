...
__global__
void compute_Fconv_fpts_2D_EulerNS(mdvector_gpu<double> Fconv, mdvector_gpu<double> U, mdvector_gpu<double> P, 
    ...
    unsigned int nFpts, double gamma)
{
  ...
  compute_Fconv_fpts_2D_EulerNS<<<blocks, threads>>>(F_fpts, U_fpts, P, nFpts, gamma);
}
